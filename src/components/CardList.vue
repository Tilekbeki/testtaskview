<template>
  ...
  <section
    ...
    <div class="sort-panel">
      <button v-on:click="sortProducts(options.id,'asc')">По возрастанию</button>
      <button v-on:click="sortProducts(options.id,'desc')">По убыванию</button>
      <button v-on:click="sortProducts(options.id,'resort')">Без сортировки</button>
    </div>
    <div class="title">
      <h2>
        {{ options.title }} 
...
<script setup>
  ...
  function sortProducts(id, typeSort) {
    let cardsBefore;
    switch (id) {
      case 1:
        cards = firstList;
